...
class SaleOrder(models.Model):
    ...
    commission_invoice_count = fields.Integer("Number of Invoice",
    ...
    def _activity_cancel_on_commission_invoice(self):
        """ If some SO are cancelled, we need to put an activity on their generated invoice. If sale lines of
            differents sale orders impact different invoice, we only want one activity to be attached.
        """
        commission_invoice_to_notify_map = {}  # map PO -> recordset of SOL as {account.invoice: set(sale.orde.liner)}

        account_invoice_lines = self.env['account.invoice.line'].search([('sale_line_id', 'in', self.mapped('order_line').ids), ('state', '!=', 'cancel')])
        for commission_invoice_line in account_invoice_lines:
            ...
            commission_invoice_to_notify_map[commission_invoice_line.order_id] |= commission_invoice_line.sale_line_id
        ...
        for commission_invoice, sale_order_lines in commission_invoice_to_notify_map.items():
            commission_invoice.activity_schedule_with_view('mail.mail_activity_data_warning',
                ...
                views_or_xmlid='sale_directsale_commission_invoice.exception_invoice_on_sale_cancellation',
                render_context={
...
class SaleOrderLine(models.Model):
    ...
    commission_invoice_line_ids = fields.One2many('account.invoice.line', 'sale_line_id', string="Generated Invoice Lines", readonly=True, help="Invoice line generated by this Sales item on order confirmation, or when the quantity was increased.")
    commission_invoice_line_count = fields.Integer("Number of generated invoice items", compute='_compute_commission_invoice_count')

    @api.multi
    @api.depends('commission_invoice_line_ids')
    def _compute_commission_invoice_count(self):
        database_data = self.env['account.invoice.line'].sudo().read_group([('sale_line_id', 'in', self.ids)], ['sale_line_id'], ['sale_line_id'])
        ...
        if self.state == 'sale' and self.product_id.type == 'service' and self.product_id.service_to_commission_invoice:
            if self.product_uom_qty < self._origin.product_uom_qty:
                ...
                warning_mess = {
                    ...
                    'message': _('You are decreasing the ordered quantity! Do not forget to manually update the invoice if needed.'),
                }
    ...
    def create(self, values):
        ...
        # Do not generate invoice when expense SO line since the product is already delivered
    ...
    def _commission_invoice_decrease_ordered_qty(self, new_qty, origin_values):
        """ Decrease the quantity from SO line will add a next acitivities on the related invoice
        ...
        last_commission_invoice_lines = self.env['account.invoice.line'].search([('sale_line_id', 'in', self.ids)])
    ...
    def _commission_invoice_increase_ordered_qty(self, new_qty, origin_values):
        """ Increase the quantity on the related invoice lines
        ...
        # TODO: Tratar STATE
        for line in self:
            last_commission_invoice_line = self.env['account.invoice.line'].search([('sale_line_id', '=', line.id)], order='create_date DESC', limit=1)
    ...
    def _commission_invoice_get_write_date(self, supplierinfo):
        """ return the ordered date for the invoice, computed as : SO commitment date - supplier delay """
    ...
    def _commission_invoice_service_prepare_order_values(self, supplierinfo):
        """ Returns the values to create the invoice from the current SO line.
    ...
    def _commission_invoice_service_prepare_line_values(self, commission_invoice, quantity=False):
        """ Returns the values to create the invoice line from the current SO line.
        ...
        # invoice line description in supplier lang
    ...
    def _commission_invoice_service_create(self, quantity=False):
        """ On Sales Order confirmation, some lines (services ones) can create a invoice line and maybe a invoice order.
            If a line should create a RFQ, it will check for existing PO. If no one is find, the SO line will create one, then adds
            a new PO line. The created invoice order line will be linked to the SO line.
            :param quantity: the quantity to force on the PO line, expressed in SO line UoM
        """
        ...
        for line in self:
            ...
            # link the generated invoice to the SO line
    ...
    def _commission_invoice_service_generation(self):
        """ Create a Invoice for the first time from the sale line. If the SO line already created a PO, it
