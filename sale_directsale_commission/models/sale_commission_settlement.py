...
# class SaleCommissionSettlement(models.Model):
    ...
    # _inherit = 'sale.ds.commission.settlement'
    # 
    # def _prepare_invoice_header(self, settlement, journal, date=False):
    #     if settlement.agent_type == 'directsale':
    ...
    #     res = super(SaleCommissionSettlement, self)._prepare_invoice_line(settlement, invoice, product)
    #     if res.get('price_unit', False) and settlement.agent_type == 'directsale':
...
class Settlement(models.Model):
    _name = "sale.ds.commission.settlement"
    ...
    def action_invoice(self):
        return {
            ...
            'name': _('Make invoice'),
            'res_model': 'sale.ds.commission.make.invoice',
            'view_type': 'form',
            'target': 'new',
            'view_mode': 'form',
...
class SettlementLine(models.Model):
    _name = "sale.ds.commission.settlement.line"
    _description = "Line of a commission settlement"

    settlement = fields.Many2one(
        "sale.ds.commission.settlement", readonly=True, ondelete="cascade",
        required=True)
    agent_line = fields.Many2many(
        comodel_name='sale.order.line.agent',
        relation='ds_settlement_agent_line_rel', column1='settlement_id',
        column2='agent_line_id', required=True)
    date = fields.Datetime(related="agent_line.order_date", store=True)
    sale_line = fields.Many2one(
        comodel_name='sale.order.line', store=True,
        related='agent_line.object_id')
    sale_order = fields.Many2one(
        comodel_name='sale.order', store=True, string="Sale Order",
        related='sale_line.order_id')
    agent = fields.Many2one(
